...
\`/getschedule\` - Shows the current posting interval.
  ...
  bot.onText(/\/addfile (.+)/, async (msg, match) => {
    ...
    try {
      ...
      bot.sendMessage(msg.chat.id, '✅ Success! Tester will use this URL on its next cycle.')
    } catch (e) {
  ...
  bot.onText(/\/removefile (.+)/, async (msg, match) => {
    ...
    bot.sendMessage(
      msg.chat.id,
      result.changes > 0 ? `✅ Success! File ID ${id} removed.` : `⚠️ Not Found.`
    )
  })
...
async function postNextConfigBatch() {
  try {
    ...
    if (initialCount > allConfigs.length)
      console.log(`[Bot] Cleaned up ${initialCount - allConfigs.length} expired configs.`)
    ...
    console.error(
      ...
      error.message.includes('ETIMEDOUT') ? 'Proxy connection timed out.' : error
    )
  }
}
