...
async function startBot() {
  try {
    ...
    console.error(
      "FATAL: Could not initialize database or create results directory.",
      error
    );
    process.exit(1);
  }
...
function setupCommandHandlers() {
  ...
  bot.onText(/\/start/, (msg) => {
    ...
    bot.sendMessage(
      msg.chat.id,
      "Welcome Admin. This bot posts configs found by the tester. Use /help for commands."
    );
  });
...
\`/getschedule\` - Shows the current posting interval.
  ...
  bot.onText(/\/addfile (.+)/, async (msg, match) => {
    if (msg.from.id !== ADMIN_USER_ID) return;
    try {
      await run("INSERT INTO config_files (url) VALUES (?)", [match[1]]);
      bot.sendMessage(
        msg.chat.id,
        "✅ Success! Tester will use this URL on its next cycle."
      );
    } catch (e) {
      bot.sendMessage(msg.chat.id, "❌ Error or URL already exists.");
    }
  });
  ...
  bot.onText(/\/listfiles/, async (msg) => {
    ...
    if (files.length === 0)
      return bot.sendMessage(msg.chat.id, "No files for tester.");
    let message = "📝 *Tester Source URLs:*\n\n";
    files.forEach((file) => {
      message += `*ID ${file.id}:* \`${file.url}\`\n`;
    });
  ...
  bot.onText(/\/setschedule (.+)/, async (msg, match) => {
    ...
    if (isNaN(seconds) || seconds < 60)
      return bot.sendMessage(msg.chat.id, "Minimum is 60 seconds.");
    await run(
      "INSERT OR REPLACE INTO settings (key, value) VALUES ('posting_interval_seconds', ?)",
      [seconds]
    );
    bot.sendMessage(
      msg.chat.id,
      `✅ Posting schedule updated to every ${seconds} seconds.`
    );
  ...
  bot.onText(/\/getschedule/, async (msg) => {
    ...
    bot.sendMessage(
      msg.chat.id,
      `⏰ Current posting schedule is ${seconds} seconds.`
    );
  });
}
...
async function postNextConfigBatch() {
  try {
    ...
    allConfigs = allConfigs.filter((c) => {
      ...
      const ageInHours = (now - testedDate) / (1000 * 60 * 60);
    ...
    if (initialCount > allConfigs.length)
      console.log(
        `[Bot] Cleaned up ${initialCount - allConfigs.length} expired configs.`
      );

    if (allConfigs.length === 0) {
      console.log("[Bot] No fresh configs available after cleanup.");
      return;
    }

    // --- 3. Sort the master list to find the best configs ---
    allConfigs.sort(
      (a, b) =>
    ...
    // --- 4. Smart Selection for the batch ---
    const batchToPost = [];

    // Always add the single best config to the batch
    if (allConfigs.length > 0) batchToPost.push(allConfigs.shift()); // .shift() removes and returns the first element
    ...
    // --- 5. Format and Post the message (same as before) ---
    const formattedConfigs = batchToPost
      .map((c) => {
        ...
        const configWithRemark = `${configPart}#${encodeURIComponent(
          remarkName
        )}`;
    ...
    // --- 6. Save the remaining configs for the next cycle ---
    if (allConfigs.length > 0) {
      ...
      console.log(
        `[Bot] Saved ${allConfigs.length} remaining configs to the pool.`
      );
    }
  } catch (error) {
    console.error(
      "[Bot] Error during posting cycle:",
...
async function startPostingCycle() {
  ...
  console.log(
    `[Bot] Posting a new batch of configs every ${intervalSeconds} seconds.`
  );
