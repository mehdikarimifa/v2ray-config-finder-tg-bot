...
// --- Configuration ---
const MAX_LATENCY_MS = process.env.MAX_LATENCY_MS
  ? parseInt(process.env.MAX_LATENCY_MS, 10)
  : 3000;
...
const SPEED_TEST_URL =
  process.env.SPEED_TEST_URL || "http://cachefly.cachefly.net/5mb.test";
...
const getArg = (argName) => {
  ...
  return argIndex > -1 && process.argv.length > argIndex + 1
...
async function initialize() {
  ...
  if (singleFilePath) {
    ...
    console.log(
      "[Tester] Starting in scheduled mode. Fetching configs from database."
    );
    await initDb();
    if (ENABLE_SPEED_TEST)
      console.log(`[Tester] Full Speed Testing is ENABLED.`);
...
function parseLink(link) {
  ...
  const namePart =
    hashIndex === -1 ? "" : decodeURIComponent(link.substring(hashIndex + 1));
  ...
  try {
    ...
    switch (protocol) {
      case "vmess":
        details = JSON.parse(
          Buffer.from(configPart.substring(8), "base64").toString()
        );
        break;
      case "vless":
      case "trojan": {
...
async function testConfig(originalLink, testPort) {
            ...
            vnext: [
              {
                ...
                port: details.port,
                users: [
                  { id: details.id, flow: details.flow, encryption: "none" },
                ],
              },
            ],
          },
  ...
  const testJson = {
    log: { loglevel: "none" },
    inbounds: [{ port: testPort, listen: "127.0.0.1", protocol: "socks" }],
    outbounds: [outboundConfig],
  };
  const tempConfigPath = `./tmp/temp_config_${crypto
    .randomBytes(4)
    .toString("hex")}.json`;
  let xrayProcess;
  ...
  try {
    ...
    if (ENABLE_SPEED_TEST) {
      ...
      if (durationSeconds > 0) {
        speedMbps = ((SPEED_TEST_FILE_SIZE_MB * 8) / durationSeconds).toFixed(
          2
        );
      }
    }
    ...
    console.log(
      `✅ [SUCCESS] (${latency}ms) | Speed: ${
        speedMbps ? speedMbps + "Mbps" : "N/A"
      } | ${geo.countryName} | ${details.ps}`
    );
    return {
      config: originalLink,
      latency,
...
async function processAndTestConfigs(configsToTest, sourceName) {
  ...
  for (let i = 0; i < configsToTest.length; i += CONCURRENT_TESTS) {
    ...
    const testPromises = batch.map((config, index) =>
      testConfig(config, 20800 + index)
    );
...
async function runSingleFileTest(filePath) {
  try {
    ...
    if (error.code === "ENOENT") {
      ...
      console.error(
        `[Tester] A critical error occurred during the single file test:`,
        error
      );
    }
    process.exit(1);
  ...
  try {
    ...
    for (const source of sources) {
      ...
      try {
        ...
        if (configs.length > 0) {
          const sourceName = new URL(source.url).pathname
            .split("/")
            .pop()
            .replace(".txt", "");
          await processAndTestConfigs(configs, sourceName);
        ...
        console.error(
          `[Tester] Failed to process source ${source.url}: ${error.message}`
        );
      }
    }
  } catch (error) {
    console.error(
      "[Tester] A critical error occurred during the test cycle:",
      error
    );
  }
}
